...
import TestPositiveReportTable from "./TestPositiveReportTable";

interface TestPositiveReportFilterProps {
  title: string;
}

const TestPositiveReportFilter = ({ title }: TestPositiveReportFilterProps) => {
  const navigate = useNavigate();
  const [fromRegistrationNo, setFromRegistrationNo] = useState("");
  const [toRegistrationNo, setToRegistrationNo] = useState("");
  const [showResults, setShowResults] = useState(false);

  const { data: testPositiveData, isLoading, refetch } = useQuery({
    queryKey: ['test-positive', fromRegistrationNo, toRegistrationNo],
    queryFn: async () => {
      ...
      let query = supabase
        ...
        `)
        .or('hbsag.gt.1.0,hcv.gt.1.0,hiv.gt.1.0,vdrl.gt.1.0');
  ...
  const handleExport = (format: 'pdf' | 'jpeg') => {
    if (!testPositiveData || testPositiveData.length === 0) {
      toast({
        title: "No Data",
        description: "No test positive data available for export.",
        variant: "destructive",
      });
      return;
    }
    ...
    const event = new CustomEvent('exportTestPositive', { 
      detail: { format, data: testPositiveData } 
    });
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-center gap-4 pt-4">
              ...
              <Button 
                ...
                disabled={!showResults || !testPositiveData || testPositiveData.length === 0}
              >
...
export default TestPositiveReportFilter;
