...
export function Sidebar({ onFormOpen }: SidebarProps) {
  ...
  const { canAccessForm, canAccessReport, canAccessUserManagement, isLoading: rolesLoading } = useRoleAccess();
  ...
  const handleReportNavigate = (path: string) => {
    if (rolesLoading) return; // Don't navigate while loading roles
  ...
  const handleFormClick = (formType: string) => {
    if (rolesLoading) return; // Don't open form while loading roles
